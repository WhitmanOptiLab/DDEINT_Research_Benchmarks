...
CONTAINS
  ...
  SUBROUTINE DDES(T,Y,Z,DY)
    ...
    ! Physical parameters
    DOUBLE PRECISION :: ca=1.55D0, cv=519.0D0, R=1.05D0,r1=0.068D0, &
    ...
    ! Derivatives:
    DY(1) = -(1.0D0/(ca*R))*Paoft + (1.0D0/(ca*R))*Pvoft + (1.0D0/ca)*Vstr*Hoft
    DY(2) =  (1.0D0/(cv*R))*Paoft - (1.0D0/(cv*R) + 1.0D0/(cv*r1))*Pvoft
